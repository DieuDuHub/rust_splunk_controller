...
const colorScale = d3.scaleSequential(d3.interpolate("#a79de6", "#031376"));
let countries = [];
let dataToShow = [];
let datafilter = null;
let max_nb_policy = 0;
	...
	for (let j = 0; j < dataset.features.length; j++) {
		for (let i = 0; i < data.markets.length; i++) {
			...
			if (row.market_code == dataset.features[j].properties.ISO_A2) {
				...
				dataset.features[j].properties['STROKECOLOR'] = "#b7b7b7";
				dataset.features[j].properties['SIDECOLOR'] = "#7c73bf";
...
const refreshGlobe = (dataset) => {
	...
	world = Globe()
		...
		.polygonCapColor(feat => (feat.properties.NBPOL === 0 || feat.properties.NBPOL === undefined) ? '#a0a0a0' : colorScale(getVal(feat)))
...
const timerefresh = () => {
	...
	*/
	...
	dataToShow.features[index].properties['STROKECOLOR'] = "#edff13";
	dataToShow.features[index].properties['SIDECOLOR'] = "#0c0063";
